...
build_model(mve::TriangleMesh::ConstPtr mesh,
    std::vector<TextureAtlas::Ptr> const & texture_atlases, ObjModel * obj_model, bool bFlg)  {
	...
	std::vector<uint64_t> label_faces;
	label_faces.resize(mesh_faces.size() / 3);//03/05/2019
	...
	if (bFlg == false)
		return;
	//texturing unreferenced faces 03/05/2019
	int count = 1;
	int n = 0;
	while (count > 0)
	{
		...
		for (int i = 0; i < label_faces.size(); ++i)
		{
			if (label_faces[i] == 1) continue;
			for (int j = 0; j < groups.size(); ++j)
			{
				int c = 0;
				std::vector<ObjModel::Face> & faces = groups[j].faces;
				for (int k = 0; k < faces.size(); ++k)
				{
					c = 0;
					for (int l = 0; l < 3; ++l)
					{
						if (mesh_faces[3 * i] == faces[k].vertex_ids[l])
						{
							++c;
							continue;
						}
						if (mesh_faces[3 * i+1] == faces[k].vertex_ids[l])
						{
							++c;
							continue;
						}
						if (mesh_faces[3 * i+2] == faces[k].vertex_ids[l])
						{
							++c;
							continue;
						}
					...
					if (c == 2)
					{
						...
						for (int l = 0; l < 3; ++l)
						{
							f.vertex_ids[l] = mesh_faces[3 * i + l];
							f.normal_ids[l] = mesh_faces[3 * i + l];
							f.texcoord_ids[l] = faces[k].texcoord_ids[l];
						}
	...
	std::cout << "increment count" << n << std::endl;
}
