...
created at 02/25/2019
*/
void ObjModel::getHoleBoundaries(std::vector<Edge> &total_boundary, std::vector<std::vector<Edge>> &boundaries)
{
...
void ObjModel::fillingHoles()
{
	...
	getHoleBoundaries(total_boundary, boundaries);
	int nLoop = 5;
...
*/
void ObjModel::extractHoleFillSamples(std::vector<Edge> &boundary, std::vector<math::Vec3f> &samples)
{
	std::vector<math::Vec3f> vts;
	float avgLen = 0.0f;
	for (int i = 0; i < boundary.size(); ++i)
	{
		math::Vec3f v1 = vertices[boundary[i].vertex_ids[0]];
		math::Vec3f v2 = vertices[boundary[i].vertex_ids[1]];
		math::Vec3f vec = v2 - v1;
		avgLen += vec.norm();
		vts.push_back(v1);
	}
	avgLen /= boundary.size();
	for (int i = 1; i < vts.size() / 2 - 1; ++i)
	{
		math::Vec3f v1 = vts[i];
		math::Vec3f v2 = vts[vts.size() - i - 1];
		math::Vec3f vec = v2 - v1;
		int n = vec.norm() / avgLen;
		vec.normalize();
		for (int j = 1; j < n; ++j)
		{
			math::Vec3f v = v1 + j*avgLen*vec;
			samples.push_back(v);
		}
	}
...
*/
void ObjModel::refiningHoles()
{
	std::vector<Edge> total_boundary;
	Triangles triangles;
	getTriangles(triangles);
	getTotalBoundaries(triangles, total_boundary);
	getHoleBoundaries(total_boundary, hole_boundaries);
	int nLoop = 5;
	Triangles newTriangles;
	while (nLoop > 0)
	{
		...
		{
			refiningBoundary(hole_boundaries[i], triangles, newTriangles);
		}
		--nLoop;
	}
	removeErrorFaces(newTriangles);
...
created at 02/28/2019
*/
void ObjModel::getHoleFillSamples(std::vector<std::vector<std::vector<float>>> &holeSamples, 
	                              std::vector<std::vector<std::vector<float>>> &holeBoundaryVertices)
{
	for (int i = 0; i < hole_boundaries.size(); ++i)
	{
		if (hole_boundaries[i].size() < 5)
			continue;
		std::vector<math::Vec3f> vecSamples;
		extractHoleFillSamples(hole_boundaries[i], vecSamples);
		std::vector<std::vector<float>> holeVertices;
		for (int j = 0; j < hole_boundaries[i].size(); ++j)
		{
			std::vector<float> p;
			p.push_back(vertices[hole_boundaries[i][j].vertex_ids[0]][0]);
			p.push_back(vertices[hole_boundaries[i][j].vertex_ids[0]][1]);
			p.push_back(vertices[hole_boundaries[i][j].vertex_ids[0]][2]);
			holeVertices.push_back(p);
		}
		holeBoundaryVertices.push_back(holeVertices);
		std::vector<std::vector<float>> samples;
		for (int j = 0; j < vecSamples.size(); ++j)
		{
			std::vector<float> p;
			p.push_back(vecSamples[j][0]);
			p.push_back(vecSamples[j][1]);
			p.push_back(vecSamples[j][2]);
			samples.push_back(p);
		}
		holeSamples.push_back(samples);
	}
}
/*
created at 02/28/2019
*/
void ObjModel::fillingHole(std::vector<std::vector<float >> &patchVerticesIn, 
						   std::vector<std::vector<float >> &patchNormalsIn, 
						   std::vector<std::vector<int>> &patchIndices)
{
	std::cout << "\t patch Input vertices count " << patchVerticesIn.size() << std::endl;
	std::vector<math::Vec3f> patchVertices;
	std::vector<math::Vec3f> patchNormals;
	for (int i = 0; i < patchVerticesIn.size(); ++i)
	{
		math::Vec3f v, normal;
		for (int j = 0; j < 3; ++j)
		{
			v[j] = patchVerticesIn[i][j];
			normal[j] = patchNormalsIn[i][j];
		}
		patchVertices.push_back(v);
		patchNormals.push_back(normal);
	}
	std::cout << "\t patch vertices count " << patchVertices.size()<< std::endl;
	for (int i = 0; i < patchVertices.size(); ++i)
	{
		int index = -1;
		for (int j = 0; j < vertices.size(); ++j)
		{
			if (patchVertices[i] == vertices[j])
			{
				index = j;
				break;
			}
		...
		if (index == -1)
		{
			vertices.push_back(patchVertices[i]);
			normals.push_back(patchNormals[i]);
			index = vertices.size() - 1;
		}
		for (int j = 0; j < patchIndices.size(); ++j)
		{
			for (int k = 0; k < 3; ++k)
			{
				if (patchVertices[i] == patchVertices[patchIndices[j][k]])
				{
					patchIndices[j][k] = index;
					break;
				}
			}
		}
	...
	for (int i = 0; i < patchIndices.size(); ++i)
	{
		...
		for (int j = 0; j < 3; ++j)
		{
			f.vertex_ids[j] = patchIndices[i][j];
			f.normal_ids[j] = patchIndices[i][j];
		}
		faces.push_back(f);
	}
	std::cout << "\t total face count " << faces.size() << std::endl;
	for (int i = 0; i < faces.size(); ++i)
	{
		for (int j = 0; j < groups.size(); ++j)
		{
			for (int k = 0; k < groups[j].faces.size(); ++k)
			{
				int count = 0;
				for (int l = 0; l < 3; ++l)
				{
					if (faces[i].vertex_ids[0] == groups[j].faces[k].vertex_ids[l])
					{
						++count;
						continue;
					}
					if (faces[i].vertex_ids[1] == groups[j].faces[k].vertex_ids[l])
					{
						++count;
						continue;
					}
					if (faces[i].vertex_ids[2] == groups[j].faces[k].vertex_ids[l])
					{
						++count;
						continue;
					}
				...
				if (count == 2)
				{
					faces[i].texcoord_ids[0] = groups[j].faces[k].texcoord_ids[0];
					faces[i].texcoord_ids[1] = groups[j].faces[k].texcoord_ids[1];
					faces[i].texcoord_ids[2] = groups[j].faces[k].texcoord_ids[2];
					groups[j].faces.push_back(faces[i]);
					break;
				}
			}
		}
	}
	std::cout << "\t group count " << groups.size() << std::endl;
}
/*
created at 03/02/2019
*/
void ObjModel::constructingPatchForHole(std::vector<Edge> &holeBoundary, std::vector<math::Vec3f> &patchVertices, 
	                                    std::vector<math::Vec3f> &patchNormals, Triangles &patchTriangles)
{
	/*if (holeBoundary.size() < 5)
		return;
	std::vector<math::Vec3f> vertSamples;
	extractHoleFillSamples(holeBoundary, vertSamples);
	...
	for (int i = 0; i < triangles.polygons.size(); ++i)
	{
		...
		for (int j = 0; j < 3; ++j)
		{
			t.vertex_ids[j] = triangles.polygons[i].vertices[j];
			t.normal_ids[j] = triangles.polygons[i].vertices[j];
		}
		patchTriangles.push_back(t);
	}*/
}
/*
created at 03/02/2019
modified at 03/04/2019
*/
void ObjModel::jointPatch(std::vector<math::Vec3f> &patchVertices, std::vector<math::Vec3f> &patchNormals, Triangles &patchTriangles)
{
	std::vector<math::Vec3i> labels;
	labels.resize(patchTriangles.size());
	std::vector<Face> patchFaces;
	for (int i = 0; i < labels.size(); ++i)
	{
		labels[i][0] = 0;
		labels[i][1] = 0;
	...
	for (int i = 0; i < patchVertices.size(); ++i)
	{
		...
		if (vtId == -1)
		{
			vertices.push_back(patchVertices[i]);
			normals.push_back(patchNormals[i]);
			vtId = vertices.size() - 1;
		}
		for (int j = 0; j < patchTriangles.size(); ++j)
		{
			for (int k = 0; k < 3; ++k)
			{
				if (labels[j][k] == 1) continue;
				if (patchTriangles[j].vertex_ids[k] == i)
				{
					patchTriangles[j].vertex_ids[k] = vtId;
					labels[j][k] = 1;
				}
			}
		}
	}
	for (int i = 0; i < patchTriangles.size(); ++i)
	{
		int groupId = -1;
		for (int j = 0; j < groups.size(); ++j)
		{
			for (int k = 0; k < groups[j].faces.size(); ++k)
			{
				...
				for (int l = 0; l < 3; ++l)
				{
					if (patchTriangles[i].vertex_ids[0] == groups[j].faces[k].vertex_ids[l])
						++count;
					if (patchTriangles[i].vertex_ids[1] == groups[j].faces[k].vertex_ids[l])
						++count;
					if (patchTriangles[i].vertex_ids[2] == groups[j].faces[k].vertex_ids[l])
						++count;
				}
				if (count == 2)
				{
		...
		patchTriangles[i].groupId = groupId;
	}
	int nCount = 1;
	while (nCount > 0)
	{
		...
		for (int i = 0; i < patchTriangles.size(); ++i)
		{
			if (patchTriangles[i].groupId > -1) continue;
			for (int j = 0; j < patchTriangles.size(); ++j)
			{
				if (i == j) continue;
				if (patchTriangles[j].groupId > -1) continue;
				int count = 0;
				for (int k = 0; k < 3; ++k)
				{
					if (patchTriangles[i].vertex_ids[0] == patchTriangles[j].vertex_ids[k])
					{
						++count;
						continue;
					}
					if (patchTriangles[i].vertex_ids[1] == patchTriangles[j].vertex_ids[k])
					{
						++count;
						continue;
					}
					if (patchTriangles[i].vertex_ids[2] == patchTriangles[j].vertex_ids[k])
					{
						++count;
						continue;
					}
				...
				if (count == 2)
				{
					patchTriangles[i].groupId = patchTriangles[j].groupId;
					++nCount;
				}
			}
		}
	...
	for (int i = 0; i < patchTriangles.size(); ++i)
	{
		if (patchTriangles[i].groupId > -1)
		{
			Face f;
			for (int j = 0; j < 3; ++j)
			{
				f.vertex_ids[j] = patchTriangles[i].vertex_ids[j];
				f.normal_ids[j] = patchTriangles[i].normal_ids[j];
				f.texcoord_ids[j] = patchTriangles[i].texcoord_ids[j];
			}
			groups[patchTriangles[i].groupId].faces.push_back(f);
		}
	}
}
/*
created at 03/02/2019
*/
void ObjModel::fillingHoles00()
{
	refiningHoles();
	for (int i = 0; i < hole_boundaries.size(); ++i)
	{
		std::vector<math::Vec3f> patchVertices, patchNormals;
		Triangles patchTriangles;
		constructingPatchForHole(hole_boundaries[i], patchVertices, patchNormals, patchTriangles);
		jointPatch(patchVertices, patchNormals, patchTriangles);
	}
}
/*
crated at 03/04/2019
*/
void ObjModel::createHoleSamples(std::vector<std::vector<std::vector<float>>> &holeSamples)
{
	std::cout << "start creating hole samples" << std::endl;
	refiningHoles();
	std::cout << "count of hole boundaries:" << hole_boundaries.size() << std::endl;
	for (int i = 0; i < hole_boundaries.size(); ++i)
	{
		if (hole_boundaries[i].size() < 5) continue;
		std::vector<std::vector<float>> samplesPerHole;
		std::vector<math::Vec3f> vertSamples;
		extractHoleFillSamples(hole_boundaries[i], vertSamples);
		std::cout << "sample count for one hole:" << hole_boundaries[i].size() << std::endl;
		for (int j = 0; j < vertSamples.size(); ++j)
		{
			std::vector<float> v;
			v.resize(3);
			v[0] = vertSamples[j][0];
			v[1] = vertSamples[j][1];
			v[2] = vertSamples[j][2];
			samplesPerHole.push_back(v);
		}
		std::cout << "00count of hole samples per hole:" << samplesPerHole.size() << std::endl;
		holeSamples.push_back(samplesPerHole);
		std::cout << "01count of hole samples per hole:" << samplesPerHole.size() << std::endl;
	}
	std::cout << "count of hole samples:" << holeSamples.size() << std::endl;
}
/*
created at 03/04/2019
*/
void ObjModel::getHolePatches(std::vector<std::vector<std::vector<float>>> &patchVertices,
							  std::vector<std::vector<std::vector<float>>> &patchNormals,
							  std::vector<std::vector<std::vector<int>>> &patchIndices)
{
	Triangles mve_patchTriangles;
	for (int i = 0; i < patchVertices.size(); ++i)
	{
		std::vector<math::Vec3f> mve_patchVertices, mve_patchNormals;
		for (int j = 0; j < patchVertices[i].size(); ++j)
		{
			math::Vec3f v, n;
			for (int k = 0; k < 3; ++k)
			{
				v[k] = patchVertices[i][j][k];
				n[k] = patchNormals[i][j][k];
			}
			mve_patchVertices.push_back(v);
			mve_patchNormals.push_back(n);
		}
		_whole_patchVertices.push_back(mve_patchVertices);
		_whole_patchNormals.push_back(mve_patchNormals);
	}
	for (int i = 0; i < patchIndices.size(); ++i)
	{
		Triangles triangles;
		for (int j = 0; j < patchIndices[i].size(); ++j)
		{
			Triangle t;
			for (int k = 0; k < 3; ++k)
			{
				t.vertex_ids[k] = patchIndices[i][j][k];
				t.normal_ids[k] = patchIndices[i][j][k];
			}
			triangles.push_back(t);
		}
		_whole_patchTriangles.push_back(triangles);
	}
}
/*
created at 03/04/2019
*/
void ObjModel::jointPatches()
{
	int nPatchCount = _whole_patchTriangles.size();
	for (int i = 0; i < nPatchCount; ++i)
	{
		jointPatch(_whole_patchVertices[i], _whole_patchNormals[i], _whole_patchTriangles[i]);
	}
}